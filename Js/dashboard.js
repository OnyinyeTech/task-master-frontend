...
function deleteTask(taskId) {
  ...
  const xhr = new XMLHttpRequest();
  xhr.open("DELETE", `http://localhost:5000/api/tasks/${taskId}`, true);
  xhr.setRequestHeader("Authorization", `Bearer ${token}`);
...
function updateTask(taskId) {
  ...
  const xhr = new XMLHttpRequest();
  xhr.open("PUT", `http://localhost:5000/api/tasks/${taskId}`, true);
  xhr.setRequestHeader("Authorization", `Bearer ${token}`);
  xhr.setRequestHeader("Content-Type", "application/json");
...
async function searchTasks() {
  ...
  try {
    ...
    xhr.open(
      "GET",
      `http://localhost:5000/api/tasks/search?keyword=${encodeURIComponent(
        keyword
      )}`,
      true
    );
...
async function filterTasks() {
  ...
  const API_URL = `http://localhost:5000/api/tasks/filter?${query}`;
